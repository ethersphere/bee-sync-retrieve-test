...
async function uploadToRandomBee(randomBee, randomBytes) {
  ...
  console.log(
    `Bee ${randomBee.url} uploaded, bytes: ${randomBytesHex}, hash ${hash}`
  );
...
function exitWithReport(code) {
  try {
    const csvLine =
      [report.startDate, report.seed, report.hash, ...report.times].join(",") +
      "\n";
...
async function uploadAndCheck() {
  const seedBytes =
    (process.argv[2] && Utils.Hex.hexToBytes(process.argv[2])) ||
